...
export default function DraftsClient() {
  ...
  async function handleDeleteDraft(draftId: string) {
    ...
    const confirmed = window.confirm("Delete this draft? This can’t be undone.");
    if (!confirmed) return;
    ...
    try {
      ...
      if (!resp.ok) {
        window.alert(json?.error || "Failed to delete draft.");
        return;
      }
  ...
  async function handleDeleteAllDrafts() {
    ...
    const confirmed = window.confirm("Delete all drafts? This can’t be undone.");
    if (!confirmed) return;
    ...
    try {
      ...
      if (!resp.ok) {
        window.alert(json?.error || "Failed to delete all drafts.");
        return;
      }

      setDrafts([]);
    } catch {
      window.alert("Failed to delete all drafts.");
    } finally {
    ...
    <main className="mx-auto max-w-2xl p-6">
      ...
      <div className="mb-4 flex items-end justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={deletingAll || Boolean(deletingId) || state === "loading"}
            type="button"
          >
            Refresh
          </button>
      ...
      {state !== "loading" && !error && sortedDrafts.length === 0 && (
        <div className="rounded-2xl border bg-white p-6 text-center shadow-sm">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Create a rewrite and tap “Save” to see it here.
          </p>
          <Link
            href="/rewrite"
